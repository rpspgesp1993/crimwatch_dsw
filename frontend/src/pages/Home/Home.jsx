...
const styleBairros = (feature) => {
  const bairro = normalize(feature.properties.NM_BAIRRO);
  const municipio = normalize(feature.properties.NM_MUN);
  const chave = `${municipio}::${bairro}`;
  const indice = densidadePorBairro[chave] || 0;

  console.log('Chave normalizada:', chave, '→ Incidência:', indice);
